...
from hacxgent.core.agents.models import (
    BUILTIN_AGENTS,
    PLAN_AGENT_TOOLS,
    AgentProfile,
    AgentSafety,
...
from hacxgent.core.paths.config_paths import ConfigPath
from hacxgent.core.paths.global_paths import GlobalPath
...
class TestAgentApplyToConfig:
    def test_custom_prompt_found_in_global_when_missing_from_project(
        ...
        """Regression test for https://github.com/hacxgentai/hacxgent/issues/288
        ...
        ~/.hacxgent/prompts/ directory and load the file from there.
        """
        project_prompts = tmp_path / "project" / ".hacxgent" / "prompts"
        project_prompts.mkdir(parents=True)

        global_prompts = tmp_path / "home" / ".hacxgent" / "prompts"
        global_prompts.mkdir(parents=True)
        (global_prompts / "cc.md").write_text("Global custom prompt")

        monkeypatch.setattr(
            "hacxgent.core.config.PROMPTS_DIR", ConfigPath(lambda: project_prompts)
        )
        monkeypatch.setattr(
            "hacxgent.core.config.GLOBAL_PROMPTS_DIR", GlobalPath(lambda: global_prompts)
        )

        base = HacxgentConfig(include_project_context=False, include_prompt_detail=False)
        agent = AgentProfile(
            name="cc",
            display_name="Cc",
            description="",
...
class TestAgentManagerCycling:
    @pytest.fixture
    def base_config(self) -> HacxgentConfig:
    ...
    def test_get_agent_order_includes_primary_agents(
        self, base_config: HacxgentConfig, backend: FakeBackend
    ) -> None:
...
class TestPlanAgentToolRestriction:
    ...
    async def test_plan_agent_only_exposes_read_tools_to_llm(self) -> None:
        ...
        config = build_test_hacxgent_config(auto_compact_threshold=0)
        agent = build_test_agent_loop(
...
class TestAgentLoopInitialization:
    def test_agent_system_prompt_id_is_applied_on_init(
        ...
        monkeypatch.setattr("hacxgent.core.agents.models.BUILTIN_AGENTS", patched_agents)
        monkeypatch.setattr("hacxgent.core.agents.manager.BUILTIN_AGENTS", patched_agents)
