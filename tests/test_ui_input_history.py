...
from hacxgent.cli.history_manager import HistoryManager
from hacxgent.cli.textual_ui.app import HacxgentApp
from hacxgent.cli.textual_ui.widgets.chat_input.body import ChatInputBody
...
async def test_ui_resumes_arrow_down_after_manual_move(
    hacxgent_app: HacxgentApp, tmp_path: Path
) -> None:
    ...
    async with hacxgent_app.run_test() as pilot:
        inject_history_file(hacxgent_app, history_path)
        chat_input = hacxgent_app.query_one(ChatInputContainer)
        textarea = chat_input.input_widget
        assert textarea is not None
