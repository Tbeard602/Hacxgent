...
from hacxgent.core.agents.models import AgentProfile, AgentSafety
...
from hacxgent.core.types import AgentStats, LLMMessage, Role, SessionMetadata
...
@pytest.fixture
def mock_hacxgent_config() -> HacxgentConfig:
    """Create a mock hacxgent config for testing."""
    return build_test_hacxgent_config(active_model="test-model", models=[], providers=[])
...
class TestSessionLoggerMetadata:
    @patch("hacxgent.core.session.session_logger.subprocess.run")
    @patch("hacxgent.core.session.session_logger.getpass.getuser")
...
class TestSessionLoggerSaveInteraction:
    ...
    async def test_save_interaction_disabled(
        ...
        result = await logger.save_interaction(
            messages=[],
            stats=AgentStats(),
            base_config=build_test_hacxgent_config(
    ...
    async def test_save_interaction_success(
        ...
        mock_hacxgent_config: HacxgentConfig,
        ...
        await logger.save_interaction(
            messages=messages,
            stats=stats,
            base_config=mock_hacxgent_config,
