...
class TestACPSetMode:
    @pytest.mark.asyncio
    async def test_set_mode_to_default(self, acp_agent_loop: HacxgentAcpAgentLoop) -> None:
    ...
    @pytest.mark.asyncio
    async def test_set_mode_to_plan(self, acp_agent_loop: HacxgentAcpAgentLoop) -> None:
        session_response = await acp_agent_loop.new_session(
            cwd=str(Path.cwd()), mcp_servers=[]
        )
        session_id = session_response.session_id
