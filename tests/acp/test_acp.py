...
def _create_hacxgent_home_dir(tmp_path: Path, *sections: dict[str, Any]) -> Path:
    """Create a temporary hacxgent home directory with a minimal config file."""
    hacxgent_home = tmp_path / ".hacxgent"
    hacxgent_home.mkdir()

    config_file = hacxgent_home / "config.toml"
    ...
    if base_config_dict.get("models"):
        for model in base_config_dict["models"]:
            if model.get("name") == "hacxgent-cli-latest":
    ...
    trusted_folters_file = hacxgent_home / "trusted_folders.toml"
    trusted_folters_file.write_text("trusted = []\nuntrusted = []", encoding="utf-8")

    return hacxgent_home


@pytest.fixture
def hacxgent_home_dir(tmp_path: Path) -> Path:
    """Create a temporary hacxgent home directory with a minimal config file."""
    return _create_hacxgent_home_dir(tmp_path)


@pytest.fixture
def hacxgent_home_grep_ask(tmp_path: Path) -> Path:
    """Create a temporary hacxgent home directory with grep configured to ask permission."""
    return _create_hacxgent_home_dir(tmp_path, {"tools": {"grep": {"permission": "ask"}}})
...
async def get_acp_agent_loop_process(
    mock_env: dict[str, str], hacxgent_home: Path
    ...
    env["HACXGENT_HOME"] = str(hacxgent_home)
...
class TestSessionManagement:
    @pytest.mark.asyncio
    async def test_multiple_sessions_unique_ids(self, hacxgent_home_dir: Path) -> None:
        mock_env = get_mocking_env(mock_chunks=[mock_llm_chunk() for _ in range(3)])
        async for process in get_acp_agent_loop_process(
            mock_env=mock_env, hacxgent_home=hacxgent_home_dir
        ):
            await send_json_rpc(
                process,
...
class TestSessionUpdates:
    ...
    @pytest.mark.asyncio
    async def test_tool_call_update_structure(self, hacxgent_home_dir: Path) -> None:
...
class TestToolCallStructure:
    ...
    async def test_tool_call_request_permission_structure(
        self, hacxgent_home_grep_ask: Path
    ) -> None:
        custom_results = [
            mock_llm_chunk(
                tool_calls=[
        ...
        async for process in get_acp_agent_loop_process(
            mock_env=mock_env, hacxgent_home=hacxgent_home_grep_ask
        ):
            session_id = await initialize_session(process)
            await send_json_rpc(
                process,
