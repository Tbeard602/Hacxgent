...
from hacxgent.core.config import ModelConfig, HacxgentConfig
from hacxgent.core.types import LLMMessage, Role


@pytest.fixture
def acp_agent_loop(backend) -> HacxgentAcpAgentLoop:
    ...
    HacxgentConfig.dump_config(config.model_dump())
    ...
    patch("hacxgent.acp.acp_agent_loop.AgentLoop", side_effect=PatchedAgentLoop).start()
...
class TestACPSetModel:
    @pytest.mark.asyncio
    async def test_set_model_success(self, acp_agent_loop: HacxgentAcpAgentLoop) -> None:
    ...
    async def test_set_model_invalid_model_returns_none(
        self, acp_agent_loop: HacxgentAcpAgentLoop
    ) -> None:
        session_response = await acp_agent_loop.new_session(
            cwd=str(Path.cwd()), mcp_servers=[]
        )
    ...
    async def test_set_model_saves_to_config(
        ...
        with patch("hacxgent.acp.acp_agent_loop.HacxgentConfig.save_updates") as mock_save:
            response = await acp_agent_loop.set_session_model(
                session_id=session_id, model_id="devstral-small"
            )
