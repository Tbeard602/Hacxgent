...
from tests.conftest import build_test_agent_loop, build_test_hacxgent_config
from tests.mock.utils import mock_llm_chunk
from tests.stubs.fake_backend import FakeBackend
from hacxgent.core.config import ModelConfig, ProviderConfig, HacxgentConfig
from hacxgent.core.llm.backend.generic import GenericBackend, OpenAIAdapter
from hacxgent.core.llm.backend.hacxgent import HacxgentBackend, HacxgentMapper, ParsedContent
from hacxgent.core.llm.format import APIToolFormatHandler
from hacxgent.core.types import AssistantEvent, LLMMessage, ReasoningEvent, Role


def make_config() -> HacxgentConfig:
...
class TestHacxgentMapperParseContent:
    def test_parse_content_string_returns_content_only(self):
        mapper = HacxgentMapper()
    ...
    def test_parse_content_text_chunk_returns_content_only(self):
        mapper = HacxgentMapper()
        content: list[hacxgentai.ContentChunk] = [
            hacxgentai.TextChunk(type="text", text="Hello from text chunk")
        ]
    ...
    def test_parse_content_thinking_chunk_extracts_reasoning(self):
        ...
        content: list[hacxgentai.ContentChunk] = [
            hacxgentai.ThinkChunk(
                type="thinking",
                thinking=[hacxgentai.TextChunk(type="text", text="Let me think...")],
            ),
            hacxgentai.TextChunk(type="text", text="The answer is 42."),
        ]
    ...
    def test_parse_content_multiple_thinking_chunks_concatenates(self):
        ...
        content: list[hacxgentai.ContentChunk] = [
            hacxgentai.ThinkChunk(
                type="thinking",
                thinking=[hacxgentai.TextChunk(type="text", text="First thought. ")],
            ),
            hacxgentai.ThinkChunk(
                type="thinking",
                thinking=[hacxgentai.TextChunk(type="text", text="Second thought.")],
            ),
            hacxgentai.TextChunk(type="text", text="Final answer."),
        ]
    ...
    def test_parse_content_thinking_only_returns_empty_content(self):
        ...
        content: list[hacxgentai.ContentChunk] = [
            hacxgentai.ThinkChunk(
                type="thinking",
                thinking=[hacxgentai.TextChunk(type="text", text="Just thinking...")],
            )
        ]
...
class TestHacxgentMapperPrepareMessage:
    def test_prepare_assistant_message_without_reasoning(self):
        ...
        assert isinstance(result, hacxgentai.AssistantMessage)
...
class TestHacxgentReasoningFieldNameValidation:
    def test_hacxgent_backend_rejects_custom_reasoning_field_name(self):
        provider = ProviderConfig(
            name="hacxgent",
        ...
        with pytest.raises(ValueError) as exc_info:
            HacxgentBackend(provider=provider)

        assert "does not support custom reasoning_field_name" in str(exc_info.value)
        assert "reasoning" in str(exc_info.value)

    def test_hacxgent_backend_accepts_default_reasoning_field_name(self):
        provider = ProviderConfig(
            name="hacxgent",
            api_base="https://api.hacxgent.ai/v1",
            api_key_env_var="MISTRAL_API_KEY",
        ...
        backend = HacxgentBackend(provider=provider)
