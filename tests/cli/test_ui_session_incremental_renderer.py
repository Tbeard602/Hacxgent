...
from hacxgent.cli.textual_ui.app import ChatScroll, HacxgentApp
from hacxgent.cli.textual_ui.widgets.load_more import (
    HistoryLoadMoreMessage,
    HistoryLoadMoreRequested,
)
from hacxgent.cli.textual_ui.widgets.messages import UserMessage
...
async def _wait_for_load_more(app: HacxgentApp, pause) -> None:
    await _wait_until(
        pause, lambda: len(app.query(HistoryLoadMoreMessage)) == 1, timeout=5.0
    )


def _load_more_remaining(app: HacxgentApp) -> int:
...
async def test_ui_session_incremental_loader_shows_tail_and_load_more(
    hacxgent_config: HacxgentConfig,
) -> None:
    agent_loop = build_test_agent_loop(config=hacxgent_config, enable_streaming=False)
    agent_loop.messages.extend([
        LLMMessage(role=Role.user, content=f"msg-{idx}") for idx in range(66)
    ])

    app = HacxgentApp(agent_loop=agent_loop, plan_offer_gateway=_pro_plan_gateway())
