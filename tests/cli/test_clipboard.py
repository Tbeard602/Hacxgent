...
from hacxgent.cli.clipboard import (
    _copy_osc52,
    _copy_primary_selection,
...
@patch("hacxgent.cli.clipboard._copy_primary_selection")
@patch("hacxgent.cli.clipboard._copy_osc52")
def test_copy_selection_to_clipboard_success(
    mock_copy_osc52: MagicMock,
    mock_copy_primary_selection: MagicMock,
    mock_app: MagicMock,
) -> None:
    widget = MockWidget(
    ...
    mock_copy_primary_selection.assert_called_once_with("selected text")
...
def test_copy_selection_to_clipboard_failure(
    ...
    mock_copy_primary_selection.side_effect = Exception("primary failed")
...
@patch("hacxgent.cli.clipboard._copy_primary_selection")
@patch("hacxgent.cli.clipboard._copy_osc52")
def test_copy_selection_to_clipboard_primary_fallback_success(
    mock_copy_osc52: MagicMock,
    mock_copy_primary_selection: MagicMock,
    mock_app: MagicMock,
) -> None:
...
def test_copy_selection_to_clipboard_multiple_widgets(mock_app: MagicMock) -> None:
    ...
    with (
        patch("hacxgent.cli.clipboard._copy_osc52") as mock_copy_osc52,
        patch("hacxgent.cli.clipboard._copy_primary_selection")
        as mock_copy_primary_selection,
    ):
        copy_selection_to_clipboard(mock_app)

        mock_copy_osc52.assert_called_once_with("first selection\nsecond selection")
        mock_copy_primary_selection.assert_called_once_with(
            "first selection\nsecond selection"
        )
        mock_app.notify.assert_called_once_with(
...
def test_copy_primary_selection_uses_xclip_when_available(
...
@patch("subprocess.run")
def test_copy_primary_selection_uses_wl_copy_on_wayland(
    mock_run: MagicMock, monkeypatch: pytest.MonkeyPatch
) -> None:
