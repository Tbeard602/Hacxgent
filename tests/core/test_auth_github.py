...
from hacxgent.core.auth.github import (
    DeviceFlowHandle,
    DeviceFlowInfo,
    GitHubAuthError,
    GitHubAuthProvider,
...
class TestGitHubAuthProviderGetToken:
    def test_returns_token_from_keyring(self) -> None:
        with patch("hacxgent.core.auth.github.keyring") as mock_keyring:
...
class TestGitHubAuthProviderStartDeviceFlow:
    ...
    async def test_start_device_flow_success(
        ...
        with patch("hacxgent.core.auth.github.webbrowser") as mock_browser:
...
class TestGitHubAuthProviderPollForToken:
    ...
    async def test_poll_returns_token_on_success(
        ...
        with patch("hacxgent.core.auth.github.asyncio.sleep", new_callable=AsyncMock):
    ...
    def test_save_token_success(self) -> None:
        with patch("hacxgent.core.auth.github.keyring") as mock_keyring:
            ...
            mock_keyring.set_password.assert_called_once_with(
                "hacxgent", "github_token", "ghp_token"
            )
...
class TestGitHubAuthProviderWaitForToken:
    ...
    async def test_wait_for_token_polls_and_saves(
        ...
        with (
            patch("hacxgent.core.auth.github.asyncio.sleep", new_callable=AsyncMock),
            patch("hacxgent.core.auth.github.keyring") as mock_keyring,
        ):
