...
from hacxgent.core.paths.config_paths import CONFIG_FILE
from hacxgent.core.paths.global_paths import GLOBAL_CONFIG_FILE, HACXGENT_HOME
...
class TestResolveConfigFile:
    def test_resolves_local_config_when_exists_and_folder_is_trusted(
        ...
        local_config_dir = tmp_path / ".hacxgent"
    ...
    def test_falls_back_to_global_config_when_local_missing(
        ...
        assert not (tmp_path / ".hacxgent" / "config.toml").exists()

        assert CONFIG_FILE.path == GLOBAL_CONFIG_FILE.path

    def test_respects_hacxgent_home_env_var(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        assert HACXGENT_HOME.path != tmp_path
        monkeypatch.setenv("HACXGENT_HOME", str(tmp_path))
        assert HACXGENT_HOME.path == tmp_path
