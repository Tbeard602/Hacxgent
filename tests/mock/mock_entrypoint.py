...
Mocked returns are stored in the HACXGENT_MOCK_LLM_DATA environment variable.
"""
...
if __name__ == "__main__":
    ...
    from hacxgent.core.types import LLMChunk

    sys.path.insert(0, str(TESTS_ROOT))

    # Apply mocking before importing any hacxgent modules
    ...
    patch(
        "hacxgent.core.llm.backend.hacxgent.HacxgentBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "hacxgent.core.llm.backend.generic.GenericBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "hacxgent.core.llm.backend.hacxgent.HacxgentBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()
    patch(
        "hacxgent.core.llm.backend.generic.GenericBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()

    from hacxgent.acp.entrypoint import main
