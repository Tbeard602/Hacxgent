...
from hacxgent.core.tools.builtins.todo import TodoArgs
...
async def test_act_flushes_batched_messages_with_injection_middleware(
    ...
    assert observed[0][1] == "You are Hacxgent, a super useful programming assistant."
...
async def test_act_streams_batched_chunks_in_order() -> None:
    backend = FakeBackend([
        ...
        mock_llm_chunk(content=" Hacxgent"),
    ...
    assert [event.content for event in assistant_events] == [
        "Hello from Hacxgent! More",
        " and end",
    ]
    assert agent.messages[-1].role == Role.assistant
    assert agent.messages[-1].content == "Hello from Hacxgent! More and end"
