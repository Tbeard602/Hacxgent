...
from hacxgent.cli.textual_ui.widgets.chat_input.completion_popup import CompletionPopup
from hacxgent.cli.textual_ui.widgets.chat_input.container import ChatInputContainer


@pytest.mark.asyncio
async def test_popup_appears_with_matching_suggestions(hacxgent_app: HacxgentApp) -> None:
    async with hacxgent_app.run_test() as pilot:
        chat_input = hacxgent_app.query_one(ChatInputContainer)
        popup = hacxgent_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_popup_hides_when_input_cleared(hacxgent_app: HacxgentApp) -> None:
    async with hacxgent_app.run_test() as pilot:
        popup = hacxgent_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_arrow_navigation_updates_selected_suggestion(hacxgent_app: HacxgentApp) -> None:
    async with hacxgent_app.run_test() as pilot:
        popup = hacxgent_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_arrow_navigation_cycles_through_suggestions(hacxgent_app: HacxgentApp) -> None:
    async with hacxgent_app.run_test() as pilot:
        popup = hacxgent_app.query_one(CompletionPopup)
...
async def test_pressing_enter_submits_selected_command_and_hides_popup(
    ...
    async with hacxgent_app.run_test() as pilot:
        ...
        assert popup.styles.display == "none"
        message = hacxgent_app.query_one(".user-command-message")
...
def file_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (tmp_path / "hacxgent" / "acp").mkdir(parents=True)
    (tmp_path / "hacxgent" / "acp" / "entrypoint.py").write_text("", encoding="utf-8")
    (tmp_path / "hacxgent" / "acp" / "agent.py").write_text("", encoding="utf-8")
...
async def test_path_completion_popup_lists_files_and_directories(
    hacxgent_app: HacxgentApp, file_tree: Path
) -> None:
    async with hacxgent_app.run_test() as pilot:
        popup = hacxgent_app.query_one(CompletionPopup)
...
async def test_fuzzy_matches_subsequence_characters(
    file_tree: Path, hacxgent_app: HacxgentApp
) -> None:
    async with hacxgent_app.run_test() as pilot:
        popup = hacxgent_app.query_one(CompletionPopup)
...
async def test_finds_files_recursively_by_filename(
    ...
    async with hacxgent_app.run_test() as pilot:
        ...
        popup_content = str(popup.render())
        assert "@hacxgent/acp/entrypoint.py" in popup_content
        assert popup.styles.display == "block"
