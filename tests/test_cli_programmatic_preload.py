...
from hacxgent.core import run_programmatic
from hacxgent.core.config import Backend
from hacxgent.core.types import LLMMessage, OutputFormat, Role
...
def test_run_programmatic_preload_streaming_is_batched(
    ...
    monkeypatch.setattr(
        "hacxgent.core.programmatic.create_formatter", lambda *_args, **_kwargs: spy
    )
    ...
    with mock_backend_factory(
        ...
        cfg = build_test_hacxgent_config(
        ...
        assert (
            spy.emitted[0][1] == "You are Hacxgent, a super useful programming assistant."
        )
