...
def test_returns_default_config_when_no_overrides(tool_manager):
    ...
    )  # due to hacxgent's discover system isinstance would fail
...
def test_merges_user_overrides_with_defaults():
    hacxgent_config = build_test_hacxgent_config(
        system_prompt_id="tests",
        include_project_context=False,
        tools={"bash": BaseToolConfig(permission=ToolPermission.ALWAYS)},
    )
    manager = ToolManager(lambda: hacxgent_config)
...
def test_preserves_tool_specific_fields_from_overrides():
    ...
    hacxgent_config.tools["bash"].__pydantic_extra__ = {"default_timeout": 600}
    manager = ToolManager(lambda: hacxgent_config)
...
from hacxgent.core.tools.base import BaseTool, BaseToolConfig, BaseToolState
...
class TestToolManagerModuleReuse:
    ...
    def test_multiple_managers_share_tool_classes(self):
        ...
        manager1 = ToolManager(lambda: hacxgent_config)
        manager2 = ToolManager(lambda: hacxgent_config)
    ...
    def test_tool_instances_are_isolated(self):
        ...
        from hacxgent.core.tools.builtins.todo import TodoItem
    ...
    def test_different_files_same_stem_get_different_modules(self, tmp_path: Path):
        ...
        ~/.config/hacxgent/tools/ should override the builtin todo.py.
        """
        import sys
