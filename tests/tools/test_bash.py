...
from hacxgent.core.tools.builtins.bash import Bash, BashArgs, BashToolConfig
...
@pytest.mark.asyncio
async def test_runs_arbitrary_command_without_builtin_filtering(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    config = BashToolConfig(allowlist=["echo"], denylist=["python"])
    bash_tool = Bash(config=config, state=BaseToolState())

    result = await collect_result(
        bash_tool.run(BashArgs(command='python3 -c "print(42)"'))
    )
...
def test_check_allowlist_denylist():
    ...
    assert allowlisted is None
    assert denylisted is None
