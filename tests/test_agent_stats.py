...
class TestReloadPreservesStats:
    ...
    async def test_reload_updates_pricing_from_new_model(self, monkeypatch) -> None:
        ...
        backend = FakeBackend(mock_llm_chunk(content="Response"))
        config_hacxgent = make_config(active_model="devstral-latest")
        agent = build_test_agent_loop(config=config_hacxgent, backend=backend)
...
class TestCompactStatsHandling:
    @pytest.mark.asyncio
    async def test_compact_preserves_recent_conversation_context(self) -> None:
        backend = FakeBackend(
            [
        ...
        async for _ in agent.act("Build something"):
            pass
        async for _ in agent.act("Continue the task"):
            pass
        ...
        assert agent.messages[-2].content == "Continue the task"
...
class TestAutoCompactIntegration:
    ...
    async def test_auto_compact_triggers_and_preserves_stats(self) -> None:
        ...
        cfg = build_test_hacxgent_config(auto_compact_threshold=1)
        agent = build_test_agent_loop(
        ...
        assert roles == [Role.system, Role.assistant, Role.user, Role.assistant]
        assert observed[1][1] is not None and "<summary>" in observed[1][1]
        assert observed[2][1] == "Hello"
