...
@pytest.mark.asyncio
async def test_passes_x_affinity_header_when_asking_an_answer(hacxgent_config: HacxgentConfig):
    backend = FakeBackend([mock_llm_chunk(content="Response")])
    agent = build_test_agent_loop(config=hacxgent_config, backend=backend)
...
async def test_passes_x_affinity_header_when_asking_an_answer_streaming(
    ...
    agent = build_test_agent_loop(
        config=hacxgent_config, backend=backend, enable_streaming=True
    )
...
@pytest.mark.asyncio
async def test_updates_tokens_stats_based_on_backend_response(hacxgent_config: HacxgentConfig):
