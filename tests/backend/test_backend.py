...
from tests.backend.data.hacxgent import (
...
from hacxgent.core.types import LLMChunk, LLMMessage, Role, ToolCall
...
class TestBackend:
    ...
    async def test_backend_complete(
        ...
        with respx.mock(base_url=base_url) as mock_api:
            ...
            BackendClasses = [
                GenericBackend,
                *([HacxgentBackend] if base_url == "https://api.hacxgent.ai" else []),
            ]
    ...
    async def test_backend_complete_streaming_error(
        self,
        base_url: Url,
        backend_class: type[HacxgentBackend | GenericBackend],
