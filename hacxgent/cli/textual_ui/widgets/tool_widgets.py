...
from hacxgent.cli.textual_ui.ansi_markdown import AnsiMarkdown as Markdown
...
from hacxgent.core.tools.builtins.bash import BashArgs, BashResult
from hacxgent.core.tools.builtins.grep import GrepArgs, GrepResult
from hacxgent.core.tools.builtins.read_file import ReadFileArgs, ReadFileResult
from hacxgent.core.tools.builtins.search_replace import (
    SEARCH_REPLACE_BLOCK_RE,
    SearchReplaceArgs,
    SearchReplaceResult,
)
from hacxgent.core.tools.builtins.todo import TodoArgs, TodoResult
from hacxgent.core.tools.builtins.write_file import WriteFileArgs, WriteFileResult
        ...
        if not self.collapsed and self.result:
            for field_name in type(self.result).model_fields:
                ...
                if value is not None and value not in ("", []):
                    value_str = str(value)
                    yield NoMarkupStatic(
                        f"{field_name}: {value_str}", classes="tool-result-detail"
                    )
...
class BashResultWidget(ToolResultWidget[BashResult]):
    def compose(self) -> ComposeResult:
        ...
        if self.collapsed:
            ...
            if stdout:
                content, truncation_info = _truncate_lines(stdout, 10)
        ...
        if stdout:
            sep = "\n" if "\n" in stdout else " "
            yield NoMarkupStatic(
                f"stdout:{sep}{stdout}", classes="tool-result-detail"
            )
        if stderr:
            sep = "\n" if "\n" in stderr else " "
            yield NoMarkupStatic(
                f"stderr:{sep}{stderr}", classes="tool-result-detail"
            )
        yield from self._footer()
...
class AskUserQuestionResultWidget(ToolResultWidget[AskUserQuestionResult]):
    def compose(self) -> ComposeResult:
        ...
        answers = self.result.answers or []
