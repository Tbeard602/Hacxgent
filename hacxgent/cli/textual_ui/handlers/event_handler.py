...
from hacxgent.cli.textual_ui.widgets.messages import AssistantMessage, ReasoningMessage
...
from hacxgent.core.tools.ui import ToolUIDataAdapter
...
from hacxgent.core.utils import TaggedText
...
class EventHandler:
    ...
    async def _handle_assistant_message(self, event: AssistantEvent) -> None:
        await self.mount_callback(AssistantMessage(event.content or ""))

    async def _handle_reasoning_message(self, event: ReasoningEvent) -> None:
        tools_collapsed = self.get_tools_collapsed()
        await self.mount_callback(
            ReasoningMessage(event.content or "", collapsed=tools_collapsed)
        )
