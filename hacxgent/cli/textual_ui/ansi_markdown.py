...
try:
    from textual.widgets import MarkdownFence
...
class AnsiMarkdownFence(MarkdownFence):
    ...
    def highlight(
        ...
        *,
        ...
        safe_code = code or ""
        safe_lexer = (lexer or "text").strip() or "text"

        try:
            highlighted = super().highlight(
                safe_code,
                safe_lexer,
                ansi=bool(ansi),
            )
        except Exception:
            # Highlighting is cosmetic; preserve the code rather than
            # crashing the entire UI.
            return (
                Text.from_ansi(safe_code)
                if ansi
                else Text(safe_code)
            )

        return highlighted if highlighted is not None else Text(safe_code)
