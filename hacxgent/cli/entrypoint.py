...
from hacxgent.core.trusted_folders import has_trustable_content, trusted_folders_manager
from hacxgent.setup.trusted_folders.trust_folder_dialog import (
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Hacxgent interactive CLI")
    parser.add_argument(
    ...
    parser.add_argument(
        "--diagnose",
        action="store_true",
        help="Print resolved runtime configuration before starting the session.",
    )
    parser.add_argument(
        "--trusted-local",
        action="store_true",
        help="Enable trusted-local execution for local tools.",
    )
    parser.add_argument(
        "--repair-shop-mode",
        action="store_true",
        help="Enable repair-shop workflow mode.",
    )
    parser.add_argument(
        "--repair-job-context",
        metavar="JSON",
        help="JSON repair-job context to seed repair-shop mode.",
    )
    parser.add_argument(
        "--agent",
        metavar="NAME",
        ...
        "or custom from ~/.hacxgent/agents/NAME.toml)",
    )
...
def check_and_resolve_trusted_folder() -> None:
    try:
        ...
        rprint(
            ...
            "[yellow]The directory you started hacxgent from has been deleted. "
...
def main() -> None:
    ...
    skip_trust_prompt = bool(args.trusted_local or args.repair_shop_mode)
    if is_interactive and not skip_trust_prompt:
        check_and_resolve_trusted_folder()
    unlock_config_paths()

    from hacxgent.cli.cli import run_cli
