...
from hacxgent.core.types import ToolStreamEvent
...
class ReadFile(
    ...
    async def _read_file(self, args: ReadFileArgs, file_path: Path) -> _ReadResult:
        def sync_read() -> _ReadResult:
            try:
                lines_to_return: list[str] = []
                bytes_read = 0
                was_truncated = False

                # Use standard synchronous file reading in a thread pool for maximum performance
