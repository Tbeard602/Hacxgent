...
class SearchReplace(
    ...
    async def _read_file(self, file_path: Path) -> str:
        try:
            # Use thread for potentially large file reads
    ...
    def _apply_blocks(
        ...
        for i, (search, replace) in enumerate(blocks, 1):
            # Normalize block line endings
            norm_search = search.replace("\r\n", "\n")
            norm_replace = replace.replace("\r\n", "\n")

            if norm_search not in current_content:
                # Try a slightly more relaxed match (ignore trailing whitespace per line)
                relaxed_search = "\n".join(l.rstrip() for l in norm_search.split("\n"))
                relaxed_content = "\n".join(l.rstrip() for l in current_content.split("\n"))
                
                if relaxed_search in relaxed_content:
                    # Found it with relaxed matching, but we need to know WHERE to replace in original
                    # For now, let's just stick to reporting error but suggesting the user check whitespace
                    pass

                context = SearchReplace._find_search_context(current_content, norm_search)
                fuzzy_context = SearchReplace._find_fuzzy_match_context(
                    current_content, norm_search, fuzzy_threshold
                )
                ...
                error_msg += (
                    ...
                    "2. The tool now normalizes line endings, so focus on content and indentation\n"
            ...
            occurrences = current_content.count(norm_search)
            if occurrences > 1:
                warning_msg = (
    ...
    def _find_search_context(
        ...
        # Try to find an anchor line that isn't just whitespace
        first_search_line = ""
        for sl in search_lines:
            if sl.strip():
                first_search_line = sl.strip()
                break
        
        if not first_search_line:
            return "Search text is empty or whitespace only"
        ...
        if not matches:
            return f"Anchor line '{first_search_line}' not found anywhere in file"
