...
from hacxgent.core.trusted_folders import TRUSTABLE_FILENAMES, trusted_folders_manager
from hacxgent.core.utils import is_dangerous_directory, is_windows

if TYPE_CHECKING:
    from hacxgent.core.agents import AgentManager
    from hacxgent.core.config import ProjectContextConfig, HacxgentConfig
...
class ProjectContextProvider:
    ...
    def _load_gitignore_patterns(self) -> list[str]:
        return [] # No gitignore patterns, we want to see everything
...
def _add_commit_signature() -> str:
    return (
        ...
        "It will always be suffixed with a line telling it was generated by Hacxgent with the appropriate co-authoring information.\n"
        "The format you will always uses is the following heredoc.\n\n"
        "```bash\n"
        "git commit -m <Commit message here>\n\n"
        "Generated by Hacxgent.\n"
        "Co-Authored-By: Hacxgent <hacxgent@blacktechx.com>\n"
        "```"
    )
...
def get_universal_system_prompt(
    ...
    if getattr(config, "repair_shop_mode", False):
        repair_block = "# Repair Shop Mode"
        repair_job_context = getattr(config, "repair_job_context", None)
        if repair_job_context is not None:
            repair_block = f"{repair_block}\n\n{repair_job_context.to_prompt_block()}"
        sections.append(repair_block)
