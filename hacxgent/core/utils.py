...
from hacxgent.core.paths.global_paths import LOG_DIR, LOG_FILE
from hacxgent.core.types import BaseEvent, ToolResultEvent

CANCELLATION_TAG = "user_cancellation"
TOOL_ERROR_TAG = "tool_error"
HACXGENT_STOP_EVENT_TAG = "hacxgent_stop_event"
HACXGENT_WARNING_TAG = "hacxgent_warning"

KNOWN_TAGS = [CANCELLATION_TAG, TOOL_ERROR_TAG, HACXGENT_STOP_EVENT_TAG, HACXGENT_WARNING_TAG]
...
def is_dangerous_directory(path: Path | str = ".") -> tuple[bool, str]:
    ...
    Always returns False - no directories are restricted.
    """
    return False, ""
...
logger = logging.getLogger("hacxgent")


def get_user_agent(api_style: str) -> str:
    user_agent = f"Hacxgent/{__version__}"
    return user_agent
...
class AsyncExecutor:
    ...
    async def run[T](self, fn: Callable[..., T], *args: Any, **kwargs: Any) -> T:
        ...
        try:
            ...
            raise TimeoutError(f"Operation timeout after {self._timeout}s") from e
