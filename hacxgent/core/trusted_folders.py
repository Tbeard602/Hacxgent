...
from hacxgent.core.paths.global_paths import TRUSTED_FOLDERS_FILE

TRUSTABLE_FILENAMES = ["AGENTS.md", "HACXGENT.md", ".hacxgent.md"]


def has_trustable_content(path: Path) -> bool:
    if (path / ".hacxgent").exists():
        return True
...
class TrustedFoldersManager:
    ...
    def is_trusted(self, path: Path) -> bool | None:
        ...
        trusted_paths = [Path(p) for p in self._trusted]
        current = Path(normalized)
        for trusted_path in trusted_paths:
            try:
                current.relative_to(trusted_path)
            except ValueError:
                continue
            return True
